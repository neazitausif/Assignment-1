...
class TokenExplorer:
    ...
    def process_input(self, text):
        """Process single input through full pipeline"""
...
def main():
    ...
    print("""
    KEY INSIGHTS FROM VIDEO WALKTHROUGH:
    
    1. TOKENIZATION DIFFERENCES:
       - TikToken (GPT-3.5/4) uses 100K vocabulary from training data
       - Qwen2 tokenizer has different vocabulary, leading to different token counts
       - Same text = different tokens depending on model's training vocabulary
    
    2. SEMANTIC UNDERSTANDING:
       - Embeddings capture MEANING, not just tokens
       - Cosine similarity measures semantic closeness (0-1 scale)
       - High scores mean semantically related, low scores mean unrelated
    
    3. CROSS-LINGUAL BEHAVIOR:
       - English model works on Chinese text
       - Similarity scores still meaningful even across languages
       - Embedding models learn universal semantic representations
    
    4. PRACTICAL APPLICATIONS:
       - Tokenization: Cost estimation, prompt engineering, context limits
       - Embeddings: Search, classification, recommendation systems
       - Both are foundation for every LLM system
    """)
